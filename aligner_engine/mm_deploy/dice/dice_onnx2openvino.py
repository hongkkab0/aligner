...
def from_onnx(onnx_model: Union[str, onnx.ModelProto],
    ...
    ov_model = openvino.convert_model(input_model=onnx_path,
                                      input=[("input", input_info.values()[0])],
                                      output=["dets", "labels"])

    openvino.save_model(ov_model, os.path.join(work_dir,"end2end.xml"), compress_to_fp16=True)
