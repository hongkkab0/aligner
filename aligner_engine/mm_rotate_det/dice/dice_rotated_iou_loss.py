...
from aligner_engine.mm_rotate_det.dice.dice_diff_iou import diff_iou_rotated_2d_cpu
...
def rotated_iou_loss(pred, target, linear=False, mode='log', eps=1e-6):
    ...
    if pred.device == torch.device('cpu'):
        ious = diff_iou_rotated_2d_cpu(pred.unsqueeze(0), target.unsqueeze(0))
    else:
...
class DiceRotatedIouLoss(nn.Module):
    """RotatedIoULoss.
    ...
    def __init__(self,
                 ...
                 loss_weight=1.0,
                 mode='log'):
        super(DiceRotatedIouLoss, self).__init__()
        assert mode in ['linear', 'square', 'log']
        if linear:
            mode = 'linear'
